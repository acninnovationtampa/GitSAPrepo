FUNCTION /1BEA/CRMB_BD_TXT_O_HD_PUT.
*"--------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
* Use generic functions to read texts for a BD head
...
* not only due to manual changes in texts
    ES_BDH-TEXT_ERROR = GC_FALSE.
    IF IS_BDH-TEXT_ERROR <> ES_BDH-TEXT_ERROR.
      EV_DATA_CHANGED = GC_TRUE.
    ENDIF.
  ELSE.
    ES_BDH-TEXT_ERROR = GC_TRUE.
    IF IS_BDH-TEXT_ERROR <> ES_BDH-TEXT_ERROR.
      EV_DATA_CHANGED = GC_TRUE.
    ENDIF.
    CASE SY-SUBRC.
