...
DATA:
  ...
  GT_DLI_REJ              TYPE /1BEA/T_CRMB_DLI_WRK,
  GT_DLI_CAN              TYPE /1BEA/T_CRMB_DLI_WRK,
...
* Event DL_RREJ0
  INCLUDE %2f1BEA%2fX_CRMBDL_RREJ00INC_F1CON.
*=====================================================================
* Implementation part
*=====================================================================
*---------------------------------------------------------------------
    ...
    CASE GV_OK_CODE.
      ...
      WHEN 'REJT'.
        ...
        IF GV_ANSWER = gc_yes.
          ...
          PERFORM DUELIST_SELECT_REJECT.
          IF GT_DLI_REJ IS INITIAL.
            MESSAGE S130(BEA).
            EXIT.
          ENDIF.
          PERFORM DUELIST_REJECT.
        ENDIF.
      WHEN 'CANC'.
        PERFORM POPUP_TO_CONFIRM_STEP CHANGING GV_ANSWER.
        IF GV_ANSWER = gc_yes.
          PERFORM DATA_INITIALIZATION.
          PERFORM DUELIST_SELECT_CANCEL.
          IF GT_DLI_CAN IS INITIAL.
            MESSAGE S130(BEA).
            EXIT.
          ENDIF.
          PERFORM DUELIST_CANCEL.
        ENDIF.
...
START-OF-SELECTION.
  ...
  PERFORM DUELIST_SELECT_REJECT.
  PERFORM DUELIST_SELECT_CANCEL.
  IF GT_DLI_REJ IS INITIAL AND
     GT_DLI_CAN IS INITIAL AND
...
* Ranges for derived items
  ...
  IF SY-BATCH IS INITIAL AND GV_OK_CODE <> 'REJT'.
  ...
  CALL FUNCTION '/1BEA/CRMB_DL_O_GETLIST'
       ...
       IMPORTING
         ET_DLI              = GT_DLI_REJ.
  LOOP AT GT_DLI_REJ INTO LS_DLI_WRK.
    LV_TABIX = SY-TABIX.
*     check authority for billing org and bill-type
    ...
    IF GV_RC NE 0.    "no authority
      DELETE GT_DLI_REJ INDEX LV_TABIX.
...
* Ranges for derived items
  ...
  IF SY-BATCH IS INITIAL AND GV_OK_CODE <> 'CANC'.
    CALL FUNCTION '/1BEA/CRMB_DL_O_GETCOUNT'
        EXPORTING
           IRT_BILL_STATUS     = LRT_BILL_STATUS
           IRT_DERIV_CATEGORY  = LRT_DERIV_CATEGORY
  ...
  CALL FUNCTION '/1BEA/CRMB_DL_O_GETLIST'
       ...
       IMPORTING
         ET_DLI              = GT_DLI_CAN.
  LOOP AT GT_DLI_CAN INTO LS_DLI_WRK.
    LV_TABIX = SY-TABIX.
*     check authority for billing org and bill-type
    PERFORM AUTHORITY_CHECK USING GC_ACTV_DISPLAY
                                  LS_DLI_WRK-BILL_ORG
                            ...
                            CHANGING GV_RC.
    IF GV_RC NE 0.    "no authority
      DELETE GT_DLI_CAN INDEX LV_TABIX.
      GV_AUTH_ERROR = GC_YES.
    ENDIF.
    IF
       LS_DLI_WRK-DERIV_CATEGORY = LS_DLI_WRK_HLP-DERIV_CATEGORY AND
...
*-------------------------------------------------------------------*
FORM DUELIST_DISPLAY.
  IF NOT GT_DLI_REJ IS INITIAL.
    APPEND LINES OF GT_DLI_REJ TO GT_DLI_WRK.
  ENDIF.
  IF NOT GT_DLI_CAN IS INITIAL.
    APPEND LINES OF GT_DLI_CAN TO GT_DLI_WRK.
  ENDIF.
  IF NOT GT_DLI_WRK IS INITIAL.
    SORT GT_DLI_WRK BY
                    LOGSYS
                    OBJTYPE
                    SRC_HEADNO.
...
FORM DUELIST_REJECT.
  SORT GT_DLI_REJ BY
                    ...
                    OBJTYPE
                    SRC_HEADNO.
*   reject superflous derived items
  CALL FUNCTION '/1BEA/CRMB_DL_O_REJECT_INCOMP'
       EXPORTING
            IT_DLI_WRK      = GT_DLI_REJ
...
FORM DUELIST_CANCEL.
    SORT GT_DLI_CAN BY
                    ...
                    OBJTYPE
                    SRC_HEADNO.
*   cancel incomplete derived items
  CALL FUNCTION '/1BEA/CRMB_DL_O_CANCEL_INCOMP'
       EXPORTING
            IT_DLI_WRK      = GT_DLI_CAN
            IV_COMMIT_FLAG  = GC_COMMIT_ASYNC
       IMPORTING
            ET_RETURN   = GT_RETURN.
